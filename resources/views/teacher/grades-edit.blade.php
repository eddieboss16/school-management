    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('teacher.grades.update', [$class->id, $assessmentType]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <input type="text" value="{{ $assessmentType }}" disabled
                                    class="block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm">
                                <p class="text-xs text-gray-500 mt-1">Assessment type cannot be changed</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="assessment_date" id="assessment_date" required
                                    value="{{ old('assessment_date', $assessmentInfo->assessment_date->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="max_score" id="max_score" step="0.01" min="1" required
                                    value="{{ old('max_score', $assessmentInfo->max_score) }}"
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                @error('max_score')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <!-- Student Grades -->
                        <div class="mb-4">
                            <h4 class="font-semibold mb-4">Edit Student Scores</h4>

                            <div class="space-y-3">
                                @foreach($class->students->sortBy('name') as $student)
                                    @php
                                        $studentGrade = $grades->get($student->id);
                                    @endphp
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex gap-4 items-center">
                                                <div>
                                                    ...
                                                    <input type="number" name="grades[{{ $studentGrade->id }}][score]" 
                                                        step="0.01" min="0" required
                                                        value="{{ old('grades.'.$studentGrade->id.'.score', $studentGrade->score) }}"
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <input type="text" name="grades[{{ $studentGrade->id }}][remarks]" 
                                                        value="{{ old('grades.'.$studentGrade->id.'.remarks', $studentGrade->remarks) }}"
                        ...
                        <div class="flex justify-between items-center pt-4 border-t">
                            <a href="{{ route('teacher.grades.view', $class->id) }}" class="text-gray-600 hover:text-gray-900">Cancel</a>
